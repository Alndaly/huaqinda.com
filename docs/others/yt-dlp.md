...
yt-dlp 是一款基于 youtube-dl 和 youtube-dlc 二次开发的命令行下的视频下载工具。
...
下面以某国漫（https://www.youtube.com/watch?v=lHvamusTCK0）为例，使用 youtube-dl 和 yt-dlp 的下载速度对比：

![yt-dlp-vs-youtube-dl-1645151899Jfx19M](https://pic2.zhimg.com/v2-4b430805810ce2b63799a503ce2f5add_r.jpg)
...
yt-dlp 的 Github 仓库地址是：https://github.com/yt-dlp/yt-dlp。

在仓库的 README 说明中有介绍各种操作系统（Unix/Mac/Windows）的安装方式。另外，也可以直接到 releases 页面下载二进制安装包进行安装。
...
对于 Windows 系统，推荐直接到 releases 页面下载二进制程序包吧。
...
![yt-dlp-v-F-1645153235ABnGuv](https://pic4.zhimg.com/v2-323377869b18e6b86c1c54abd0b8e85f_r.jpg)
...
下面的示例都以 MIT 6.824: Distributed Systems 第十三节课（https://www.youtube.com/watch?v=4eW5SWBi7vs）为例做说明。
...
![yt-dlp-embed-subtitle-to-video](https://pic1.zhimg.com/v2-0a5ad42c8e0ff0f0799e50191db49c10_r.jpg)
...
下面是一些常用的模板变量，你可以根据需要自由组合这些变量来构建输出文件名的模板。要查看 yt-dlp 的完整模板变量列表和详细信息，你可以查看 yt-dlp 的官方文档（https://github.com/yt-dlp/yt-dlp#output-template）。文档通常包含有关每个变量的用法和示例。
...
```shell
yt-dlp --cookies cookies_svip.txt --embed-thumbnail -f bestvideo+bestaudio -o "P%(playlist_index)02d｜%(title)s.%(ext)s" 视频链接GitHub - yt-dlp/yt-dlp: A youtube-dl fork with additional features and fixesyt-dlp --cookies cookies_svip.txt --embed-thumbnail -f bestvideo+bestaudio -o "P%(playlist_index)02d｜%(title)s.%(ext)s" 视频链接
```